...
export function PrivadoWorkspacePage() {
  ...
  const incidentesGeo = incidentesPrivados.filter((incidente: any) => incidente.latitud && incidente.longitud).length;
  const geocodificacion = pct(Number(op.incidentes_geocodificados || 0), Number(op.incidentes || 0));
  ...
  return (
    <div className="flex h-screen bg-background text-foreground">
      <aside className="hidden w-[280px] shrink-0 flex-col border-r border-border bg-card lg:flex">
        <div className="border-b border-border px-5 py-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="atalaya-serif text-xl font-semibold leading-none">Atalaya Privado</div>
              <div className="atalaya-kicker text-[9px]">Operacion corporativa</div>
            </div>
          </div>
        </div>

        <nav className="flex-1 p-4">
          <div className="atalaya-kicker mb-2 px-1">Workspace</div>
          <div className="space-y-1">
            {[
              ['Operacion privada', Activity],
              ['Fuentes de datos', Database],
              ['Carga operacional', Upload],
              ['Cobertura sedes', MapPin],
            ].map(([label, Icon]: any, index) => (
              <button
                key={label}
                className={`flex w-full items-center gap-2.5 rounded-sm border-l-2 px-3 py-2 text-sm ${
                  index === 0 ? 'border-l-primary bg-muted font-medium' : 'border-l-transparent text-muted-foreground hover:bg-muted'
                }`}
              >
        ...
        <div className="border-t border-border p-4">
          <div className="atalaya-kicker mb-2">Vertical activa</div>
          <select
            value={vertical}
            onChange={(event) => setVertical(event.target.value)}
            className="w-full rounded-sm border border-border bg-background px-3 py-2 text-sm focus:outline-none"
          >
            {VERTICALES.map((item) => (
      ...
      <main className="min-w-0 flex-1 overflow-auto">
        <header className="sticky top-0 z-20 border-b border-border bg-card/95 px-4 py-4 backdrop-blur lg:px-6">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <h1 className="atalaya-serif text-2xl font-semibold">Centro de operaciones privadas</h1>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <Link to="/dashboard" className="inline-flex h-9 items-center gap-2 rounded-sm border border-border px-3 text-sm font-medium hover:bg-muted lg:hidden">
                <ArrowLeft className="h-4 w-4" />
                Municipal
              </Link>
              <select
                value={vertical}
                onChange={(event) => setVertical(event.target.value)}
                className="h-9 rounded-sm border border-border bg-background px-3 text-sm focus:outline-none lg:hidden"
              >
                {VERTICALES.map((item) => (
                  <option key={item.value} value={item.value}>{item.label}</option>
                ))}
              ...
              <div className="inline-flex h-9 items-center gap-2 rounded-full bg-accent px-3">
                ...
                <span className="atalaya-mono text-[10px] uppercase text-muted-foreground">Datos privados listos</span>
              </div>
            </div>
          </div>
        </header>
            ...
            <div className="border border-border bg-card">
              <div className="flex items-center justify-between border-b border-border px-4 py-3">
                <div>
                  ...
                  <div className="text-sm text-muted-foreground">{incidentesGeo}/{incidentesPrivados.length || 0} con coordenadas</div>
                </div>
              ...
              <div className="overflow-x-auto">
                ...
                {!incidentesPrivados.length && (
                  <div className="p-10 text-center text-sm text-muted-foreground">Carga organizaciones, sedes e incidentes para activar la operacion privada.</div>
                )}
              </div>
            </div>
            ...
            <div className="space-y-5">
              <div className="border border-border bg-card p-4">
                <div className="mb-3 flex items-center justify-between">
                  <div className="atalaya-kicker">Carga CSV</div>
              ...
              <div className="border border-border bg-card p-4">
                <div className="atalaya-kicker mb-3">Calidad operacional</div>
                {[
                  ['Sedes geocodificadas', pct(sedesGeo, sedesPrivadas.length)],
                  ['Incidentes geocodificados', geocodificacion],
                  ['Fuentes recomendadas', fuentes.length ? 100 : 0],
                ].map(([label, value]: any) => (
